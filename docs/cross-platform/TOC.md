# [Cross-Platform](index.yml)
## [Wprowadzenie](get-started/index.md)
### [Wymagania](get-started/requirements.md)
### [Instalacja](get-started/installation/index.md)
#### [Instalowanie środowiska Xamarin w programie Visual Studio w systemie Windows](get-started/installation/windows.md)
#### [Instalowanie programu Visual Studio dla komputerów Mac](/visualstudio/mac/installation/)
#### [Instrukcje dotyczące konfigurowania zapory środowiska Xamarin](get-started/installation/firewall.md)
#### [Odinstalowywanie środowiska Xamarin](get-started/installation/uninstalling-xamarin.md)
### [Wprowadzenie do tworzenia aplikacji dla urządzeń przenośnych](get-started/introduction-to-mobile-development.md)
### [Wprowadzenie do usługi SDLC dla urządzeń przenośnych](get-started/introduction-to-mobile-sdlc.md)
## [Podstawy aplikacji](app-fundamentals/index.md)
### [Ułatwienia dostępu](app-fundamentals/accessibility.md)
### [Tworzenie aplikacji międzyplatformowych](app-fundamentals/building-cross-platform-applications/index.md)
#### [Omówienie](app-fundamentals/building-cross-platform-applications/part-0-overview.md)
#### [Część 1. Opis platformy Xamarin dla urządzeń przenośnych](app-fundamentals/building-cross-platform-applications/part-1-understanding-the-xamarin-mobile-platform.md)
#### [Część 2. Architektura](app-fundamentals/building-cross-platform-applications/part-2-architecture.md)
#### [Część 3. Konfigurowanie międzyplatformowego rozwiązania Xamarin](app-fundamentals/building-cross-platform-applications/part-3-setting-up-a-xamarin-cross-platform-solution.md)
#### [Część 4. Praca z wieloma platformami](app-fundamentals/building-cross-platform-applications/part-4-platform-divergence-abstraction-divergent-implementation.md)
#### [Część 5. Praktyczne strategie udostępniania kodu](app-fundamentals/building-cross-platform-applications/part-5-practical-code-sharing-strategies.md)
#### [Część 6. Testowanie i zgody sklepu App Store](app-fundamentals/building-cross-platform-applications/part-6-testing-and-app-store-approvals.md)
#### [Analiza przypadku: Tasky](app-fundamentals/building-cross-platform-applications/case-study-tasky.md)
### [Opcje udostępniania kodu](app-fundamentals/code-sharing.md)
### [Biblioteki klas przenośnych](app-fundamentals/pcl.md)
### [Projekty udostępnione](app-fundamentals/shared-projects.md)
### [.NET Standard](app-fundamentals/net-standard.md)
### [Pakiety NuGet (automatyczne)](app-fundamentals/nuget-multiplatform-libraries/index.md)
#### [Istniejące projekty bibliotek](app-fundamentals/nuget-multiplatform-libraries/existing-library.md)
#### [Nowa biblioteka wieloplatformowa](app-fundamentals/nuget-multiplatform-libraries/single-codebase.md)
#### [Nowe biblioteki specyficzne dla platformy](app-fundamentals/nuget-multiplatform-libraries/platform-specific.md)
#### [Przewodnik po metadanych](app-fundamentals/nuget-multiplatform-libraries/metadata.md)
### [Pakiety NuGet (ręczne)](app-fundamentals/nuget-manual.md)
### [Lokalizacja](app-fundamentals/localization.md)
### [Protokół Transport Layer Security (TLS)](app-fundamentals/transport-layer-security.md)
## [Funkcje platformy](platform/index.md)
### [Asynchroniczna pomoc techniczna — omówienie](platform/async.md)
### [Funkcje języka C# 6](platform/csharp-six.md)
### [F#](platform/fsharp/index.md)
#### [Wprowadzenie](platform/fsharp/overview.md)
#### [Przykłady języka F#](platform/fsharp/samples.md)
### [Visual Basic.NET](platform/visual-basic/index.md)
#### [Rozszerzenia środowiska Xamarin dla systemów iOS i Android](platform/visual-basic/native-apps.md)
#### [Xamarin.Forms](platform/visual-basic/xamarin-forms.md)
### [Szablony Razor HTML](platform/razor-html-templates/index.md)
## [Usługi danych i usługi w chmurze](data-cloud/index.md)
### [Usługi połączone](data-cloud/connected-services.md)
### [Azure Active Directory](data-cloud/active-directory/index.md)
#### [Wprowadzenie](data-cloud/active-directory/get-started/index.md)
##### [Krok 1. Rejestracja](data-cloud/active-directory/get-started/register.md)
##### [Krok 2. Konfiguracja](data-cloud/active-directory/get-started/configure.md)
#### [Uzyskiwanie dostępu do interfejsu API programu Graph](data-cloud/active-directory/graph.md)
### [Azure Mobile Apps](data-cloud/mobile-apps.md)
### [Wprowadzenie do usług sieci Web](data-cloud/web-services/index.md)
#### [Przewodnik — Praca z technologią WCF](data-cloud/web-services/walkthrough-working-with-wcf.md)

## [Wdrażanie i testowanie](deploy-test/index.md)
### [Konfiguracja konsolidatora niestandardowego](deploy-test/linker.md)
### [Wydajność](deploy-test/memory-perf-best-practices.md)
### [Instalowanie struktury NUnit 2.6.4 przy użyciu narzędzia NuGet](deploy-test/installing-nunit-using-nuget.md)
### [Debugowanie wielu procesów](deploy-test/multi-process-debugging.md)
## [Platforma firmy Apple](macios/index.md)
### [Zarządzanie kontem Apple](macios/apple-account-management.md)
### [Ujednolicony interfejs API](macios/unified/index.md)
#### [Omówienie](macios/unified/overview.md)
#### [Aktualizowanie istniejących aplikacji](macios/unified/updating-apps.md)
#### [Aktualizowanie istniejących aplikacji dla systemu iOS](macios/unified/updating-ios-apps.md)
#### [Aktualizowanie istniejących aplikacji dla komputerów Mac](macios/unified/updating-mac-apps.md)
#### [Aktualizowanie istniejących aplikacji rozszerzenia Xamarin.Forms](macios/unified/updating-xamarin-forms-apps.md)
#### [Migrowanie powiązania do ujednoliconego interfejsu API](macios/unified/update-binding.md)
#### [Porady dotyczące aktualizowania kodu na potrzeby ujednoliconego interfejsu API](macios/unified/updating-tips.md)
### [Tworzenie powiązań języka Objective-C](macios/binding/index.md)
#### [Omówienie](macios/binding/overview.md)
#### [Tworzenie powiązań bibliotek języka Objective-C](macios/binding/objective-c-libraries.md)
#### [Tworzenie powiązań odwołań do typów](macios/binding/binding-types-reference.md)
#### [Narzędzie Objective Sharpie](macios/binding/objective-sharpie/index.md)
##### [Wprowadzenie](macios/binding/objective-sharpie/get-started.md)
##### [Narzędzia i polecenia](macios/binding/objective-sharpie/tools.md)
##### [Funkcje](macios/binding/objective-sharpie/platform/index.md)
###### [Definicje interfejsu API oraz struktury i typy wyliczeniowe](macios/binding/objective-sharpie/platform/apidefinitions-structsandenums.md)
###### [Tworzenie powiązań struktur natywnych](macios/binding/objective-sharpie/platform/native-frameworks.md)
###### [Weryfikowanie atrybutów](macios/binding/objective-sharpie/platform/verify.md)
##### [Przykłady](macios/binding/objective-sharpie/examples/index.md)
###### [Xcode](macios/binding/objective-sharpie/examples/xcode.md)
###### [CocoaPod](macios/binding/objective-sharpie/examples/cocoapod.md)
###### [Zaawansowane (ręczne)](macios/binding/objective-sharpie/examples/advanced.md)
##### [Historia wersji](macios/binding/objective-sharpie/releases.md)
#### [Rozwiązywanie problemów](macios/binding/troubleshooting.md)
### [Odwołania natywne](macios/native-references.md)
### [Typy natywne](macios/nativetypes.md)
### [Zagadnienia dotyczące platformy 32- i 64-bitowej](macios/32-and-64/index.md)
#### [Aktualizowanie ujednoliconych aplikacji Xamarin.Mac do wersji 64-bitowej](macios/32-and-64/mac-64-bit.md)
### [Praca z typami natywnymi w aplikacjach międzyplatformowych](macios/native-types-cross-platform.md)
### [Stos HttpClient i selektor implementacji protokołów SSL/TLS](macios/http-stack.md)
## [Elementy wewnętrzne](internals/index.md)
### [Dostępne zestawy](internals/available-assemblies.md)
## [Rozwiązywanie problemów](troubleshooting/index.md)
### [Często zadawane pytania](troubleshooting/questions/index.md)
#### [Czy mogę używać programu Visual Studio 2017 Release Candidate ze środowiskiem Xamarin?](troubleshooting/questions/visualstudio-2017-rc.md)
#### [Jak wyświetlić biblioteki obsługiwane w aplikacji PCL?](troubleshooting/questions/pcl-support-libraries.md)
#### [Interfejs API odbicia aplikacji PCL](troubleshooting/questions/pcl-reflection.md)
#### [Analiza przypadku aplikacji PCL: jak rozwiązać problemy związane z obiektem System.Diagnostics.Tracing dla pakietu NuGet przepływu danych TPL firmy Microsoft?](troubleshooting/questions/pcl-case-study.md)
#### [Jak można zaktualizować narzędzie NuGet?](troubleshooting/questions/nuget-update.md)
#### [Jak obniżyć wersję pakietu NuGet?](troubleshooting/questions/nuget-package-downgrade.md)
#### [Po zaktualizowaniu pakietów Nuget jest wyświetlany błąd dotyczący brakujących pakietów](troubleshooting/questions/nuget-packages-missing.md)
#### [Ujednolicanie składników usług Google Play i pakietów NuGet](troubleshooting/questions/gps-components-nuget.md)
#### [Gdzie są przechowywane składniki na moim komputerze?](troubleshooting/questions/component-storage.md)
#### [Gdzie mogę znaleźć informacje o wersji i dzienniki?](troubleshooting/questions/version-logs.md)
#### [Kiedy i jak należy zgłosić raport o usterce?](troubleshooting/questions/howto-file-bug.md)
#### [Dlaczego system Jenkins nie jest obsługiwany przez środowisko Xamarin?](troubleshooting/questions/xamarin-jenkins.md)
#### [Jakie ustawienia projektu są wymagane dla debugera?](troubleshooting/questions/debugger-settings.md)
### [Aktualizowanie odwołań do składników na potrzeby narzędzia NuGet](troubleshooting/component-nuget.md)
### [Opcje pomocy technicznej](troubleshooting/support-options.md)
### [Pytania dotyczące starszych licencji](troubleshooting/legacy-licenses/index.md)
#### [Zarządzanie zespołem platformy Xamarin](troubleshooting/legacy-licenses/team-management.md)
#### [Jak zmienić adres e-mail swojego konta?](troubleshooting/legacy-licenses/change-email.md)
#### [Jak ręcznie zsynchronizować ponownie licencje środowiska Xamarin?](troubleshooting/legacy-licenses/resync-licenses.md)
#### [Niektóre określone błędy licencjonowania](troubleshooting/legacy-licenses/licensing-errors.md)
#### [Dlaczego nie mogę zalogować się do środowiska Xamarin w programie Visual Studio lub Visual Studio dla komputerów Mac?](troubleshooting/legacy-licenses/login-troubleshooting.md)
## [Przykłady](samples/index.yml)
