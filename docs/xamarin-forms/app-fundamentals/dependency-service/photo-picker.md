...
description: W tym artykule opisano sposób użycia klasy DependencyService platformy Xamarin.Forms do pobrania zdjęcia z biblioteki obrazów przez telefon.
ms.prod: xamarin
...
Implementacja systemu iOS `IPicturePicker` używa interfejsu [ `UIImagePickerController` ](https://developer.xamarin.com/api/type/UIKit.UIImagePickerController/) zgodnie z opisem w [ **Wybierz zdjęcie z galerii** ](https://developer.xamarin.com/recipes/ios/media/video_and_photos/choose_a_photo_from_the_gallery/) przepisu i [przykładowy kod](https://github.com/xamarin/recipes/tree/master/Recipes/ios/media/video_and_photos/choose_a_photo_from_the_gallery).
...
Ta metoda uzyskuje dostęp do `MainActivity` klasy do kilku celów: dla `Instance` właściwości, dla `PickImageId` dla pole `TaskCompletionSource` właściwości oraz wywołanie `StartActivityForResult`. Ta metoda jest definiowana za pomocą `FormsApplicationActivity` klasy oznacza to klasa podstawowa `MainActivity`.
...
Teraz, gdy interfejs został zaimplementowany dla każdej platformy, aplikacji w bibliotece programu .NET Standard mogą korzystać z niego.
